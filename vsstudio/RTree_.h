...
//    int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool(const DATATYPE&, std::vector<int>&)> callback, std::vector<int>& result) const;
//
...
//    bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool(const DATATYPE&, std::vector<int>&)> callback,  std::vector<int>& result) const;
...
//
//RTREE_TEMPLATE
//int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool(const DATATYPE&, std::vector<int>&)> callback, std::vector<int>& result) const
//{
//#ifdef _DEBUG
//    for (int index = 0; index < NUMDIMS; ++index)
//    {
...
//
//    int foundCount = 0;
//    Search(m_root, &rect, foundCount, callback,result);
//
//    return foundCount;
//}
//
...
//RTREE_TEMPLATE
//bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool(const DATATYPE&,std::vector<int>&)> callback, std::vector<int>& result) const
//{
//    ASSERT(a_node);
//    ASSERT(a_node->m_level >= 0);
//    ASSERT(a_rect);
...
//            if (Overlap(a_rect, &a_node->m_branch[index].m_rect))
//            {
//                if (!Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, callback,result))
//                {
